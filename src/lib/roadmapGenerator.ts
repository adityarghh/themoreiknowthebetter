...
const getGeminiClient = () => {
  ...
  if (!apiKey) {
    console.warn("GEMINI_API_KEY is missing. Using fallback roadmap generator.");
  }
...
export async function generateOrGetRoadmap(
  topic: string,
  depth: DepthOption
): Promise<{ graph: RoadmapGraph; cached: boolean }> {
  ...
  switch (depth) {
    case "curious":
      ...
      totalTime = "≈ 30 mins";
      depthPromptText =
        "Just Curious mode: Create 5 to 7 high-level core foundational nodes that give a fast, crisp overview of the topic in 30 minutes.";
      break;
    case "learn":
      targetNodes = 12;
      totalTime = "≈ 12 hrs";
      depthPromptText =
        "Learn mode: Create 10 to 14 structured, practical nodes covering foundational concepts up to working proficiency in ~12 hours.";
      break;
    case "master":
      targetNodes = 20;
      totalTime = "≈ 80 hrs";
      depthPromptText =
        "Master mode: Create 18 to 24 comprehensive nodes covering core fundamentals, advanced techniques, architecture/theories, and practical mastery in ~80 hours.";
      break;
    case "rabbithole":
      targetNodes = 32;
      totalTime = "≈ 250+ hrs";
      depthPromptText =
        "Rabbit Hole mode: Create an exhaustive, deep dependency graph with 28 to 38 nodes covering every prerequisite, historical context, underlying theory, advanced specialization, edge cases, and expert topics.";
      break;
  }
...
Return ONLY JSON conforming strictly to the requested schema.`;
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            summaryOverview: {
              type: Type.STRING,
              description: "A crisp 2-sentence summary of what learning this topic entails.",
            },
            ...
            nodes: {
              ...
              items: {
                ...
                properties: {
                  ...
                  importance: {
                    type: Type.STRING,
                    description: "Core Foundation | Essential | Specialized | Deep Dive",
                  },
                  prerequisites: {
                    type: Type.ARRAY,
                  ...
                  resources: {
                    ...
                    items: {
                      ...
                      properties: {
                        type: {
                          type: Type.STRING,
                          description: "youtube | article | docs | book | practice",
                        },
                  ...
                  studyGuideMarkdown: {
                    type: Type.STRING,
                    description: "Detailed markdown study guide / notes for this concept",
                  },
                },
                required: [
                  "id",
    ...
    const formattedNodes: RoadmapNodeData[] = parsedData.nodes.map((n: any, idx: number) => {
      ...
      const resources = (n.resources || []).map((r: any) => {
        ...
        if (r.type === "youtube" && (!cleanUrl || !cleanUrl.startsWith("http"))) {
          cleanUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(
            normalizedTopic + " " + n.title
          )}`;
        } else if (!cleanUrl || !cleanUrl.startsWith("http")) {
          cleanUrl = `https://www.google.com/search?q=${encodeURIComponent(
            normalizedTopic + " " + n.title + " tutorial"
          )}`;
        }
        return {
          type: r.type || "article",
      ...
      return {
        ...
        difficulty: (
          ["Beginner", "Intermediate", "Advanced", "Expert"].includes(n.difficulty)
            ? n.difficulty
            : "Beginner"
        ) as any,
        importance: (
          ["Core Foundation", "Essential", "Specialized", "Deep Dive"].includes(n.importance)
            ? n.importance
            : "Essential"
        ) as any,
    ...
    const graph: RoadmapGraph = {
      ...
      summaryOverview:
        parsedData.summaryOverview ||
        `A visual, hand-drawn dependency roadmap to learn ${normalizedTopic} effectively.`,
    };
...
export function generateFallbackRoadmap(topic: string, depth: DepthOption): RoadmapGraph {
  const sampleNodesCount =
    depth === "curious" ? 6 : depth === "learn" ? 12 : depth === "master" ? 18 : 25;
  ...
  return {
    ...
    depth: depth,
    estimatedTotalTime:
      depth === "curious" ? "≈ 30 mins" : depth === "learn" ? "≈ 12 hrs" : depth === "master" ? "≈ 80 hrs" : "≈ 200+ hrs",
    createdAt: new Date().toISOString(),
    nodes: nodes,
    progress: {
      completedCount: 0,
